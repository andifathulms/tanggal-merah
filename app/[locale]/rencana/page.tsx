...
export function generateMetadata({ params }: { readonly params: { readonly locale: string } }): Metadata {
  ...
  return metadataHalaman(locale, 'rencana')
}
...
export default function RencanaPage({ params }: { readonly params: { readonly locale: string } }) {
  const locale = isLocale(params.locale) ? params.locale : LOCALE_DEFAULT
  return (
    <>
      <Nav locale={locale} halaman="rencana" />
      <Perencana locale={locale} tampilkan="rencana" />
    </>
  )
}
